...
parser.add_argument('--train-image-dir', type=str,
                    default="sample_data/training/", help="training images")
parser.add_argument('--test-image-dir', type=str,
                    default="sample_data/test/", help="test images")
parser.add_argument('--label-dir', type=str,
                    default="sample_data/labels/", help="labels for train/test")
...
parser.add_argument('--pos-weight', type=float, default=1.0,
                    help='positive class weight in loss. 1.0 = balanced')
parser.add_argument('--run', type=str, required=True,
                    help="run dir for tb & ckpts")
parser.add_argument('--no-use-skip-connections',
                    action='store_true', help='set to disable skip connections')
parser.add_argument('--no-use-batch-norm', action='store_true',
                    help='set to disable batch norm')
parser.add_argument('--base-filter-size', type=int, default=8, help=' ')
parser.add_argument('--flip-left-right', action='store_true',
                    help='randomly flip training egs left/right')
parser.add_argument('--random-rotate', action='store_true',
                    help='randomly rotate training images')
parser.add_argument('--steps', type=int, default=100000,
                    help='max number of training steps (summaries every --train-steps)')
parser.add_argument('--train-steps', type=int, default=100,
                    help='number training steps between test and summaries')
...
parser.add_argument('--connected-components-threshold',
...
print("num_test_files=", num_test_files, "batch_size=",
      opts.batch_size, "=> num_test_steps=", num_test_steps)
...
train_summaries_writer = tf.compat.v1.summary.FileWriter(
    "tb/%s/training" % opts.run, None)
test_summaries_writer = tf.compat.v1.summary.FileWriter(
    "tb/%s/test" % opts.run, None)
...
while not done:
    ...
    # ...train
    train_summaries_writer.add_summary(
        u.explicit_summaries({"xent": train_loss}), step)
...
#  train_summaries_writer.add_summary(debug_img_summary, step)
    ...
    # ... test
    stats = test.pr_stats(opts.run, opts.test_image_dir,
                          opts.label_db, opts.connected_components_threshold)
    ...
    for idx, img in enumerate(stats['debug_imgs']):
        debug_img_summary = u.pil_image_to_tf_summary(
            img, tag="debug_img_%d" % idx)
    ...
    log.append("test stats { p:%0.2f, r:%0.2f, f1:%0.2f }" % tuple(
        [stats[k] for k in ['precision', 'recall', 'f1']]))
    print("\t".join(log))
