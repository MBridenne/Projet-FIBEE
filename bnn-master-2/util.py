...
def xys_to_bitmap(xys, height, width, rescale=1.0):
    ...
    bitmap = np.zeros(
        (int(height*rescale), int(width*rescale), 1), dtype=np.float32)
    for x, y in xys:
        try:
            # recall images are (height, width)
...
def explicit_summaries(tag_values):
    values = [tf.compat.v1.Summary.Value(
        tag=tag, simple_value=value) for tag, value in tag_values.items()]
    return tf.compat.v1.Summary(value=values)
...
def pil_image_to_tf_summary(img, tag="debug_img"):
    ...
    return tf.compat.v1.Summary(value=[tf.compat.v1.Summary.Value(tag=tag,
                                                                  image=tf.compat.v1.Summary.Image(height=img.size[0],
                                                                                                   width=img.size[1],
                                                                                                   colorspace=3,  # RGB
...
def check_images(fnames):
    ...
    for i, fname in enumerate(fnames):
        ...
        if i == 0:
            ...
            print("Image size does not match others:",
                  fname, "wh:", width, height)
            exit()
    return width, height
