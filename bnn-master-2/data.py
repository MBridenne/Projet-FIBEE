...
def img_xys_iterator(image_dir, label_dir, batch_size, patch_width_height, distort_rgb,
    ...
    if patch_set and width_height_set:
        raise Exception(
            "need to set either --patch-width-height or --width and --height, not both")
    if not (patch_set or width_height_set):
        raise Exception(
            "need to set one of either --patch-width-height or both --width and --height")
    ...
    def random_crop(rgb, bitmap):
        ...
        offset_height = tf.random.uniform(
            [], 0, height-patch_height, dtype=tf.int32)
        offset_width = tf.random.uniform(
            [], 0, width-patch_width, dtype=tf.int32)
        rgb = tf.image.crop_to_bounding_box(
            rgb, offset_height, offset_width, patch_height, patch_width)
        rgb = tf.reshape(rgb, (patch_height, patch_width, 3))
        # TODO: remove this cast uglyness :/
        bitmap = tf.image.crop_to_bounding_box(bitmap,
                                               tf.cast(
                                                   tf.cast(offset_height, tf.float32) * label_rescale, tf.int32),
                                               tf.cast(
                                                   tf.cast(offset_width, tf.float32) * label_rescale, tf.int32),
                                               int(patch_height * label_rescale), int(patch_width * label_rescale))
        bitmap = tf.reshape(
            bitmap, (int(patch_height * label_rescale), int(patch_width * label_rescale), 1))
        return rgb, bitmap
    ...
    def augment(rgb, bitmap):
        ...
        if distort_rgb:
            ...
            rgb = tf.clip_by_value(
                rgb, clip_value_min=-1.0, clip_value_max=1.0)

        if random_rotation:
            # we want to use the same crop for both RGB input and bitmap labels
            random_rotation_angle = tf.random.uniform(
                [], -0.4, 0.4, dtype=tf.float32)
    ...
    def set_explicit_size(rgb, bitmap):
        if height is None or width is None:
            raise Exception(
                ">set_explicit_size requires explicit height/width set when not patch sampling")
    ...
    if repeat:
        ...
        print("len(rgb_filenames)", len(rgb_filenames),
              ("CACHE" if len(rgb_filenames) < 1000 else "NO CACHE"))
...
if __name__ == "__main__":
    ...
    imgs, xyss = tf.compat.v1.data.make_one_shot_iterator(imgs_xyss).get_next()
